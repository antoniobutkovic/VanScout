...
- `services/s3_storage.ts` — file storage through the platform OSS proxy:
  ...
  `storageGetByPath(path)`, `storageGetDownloadUrl(id)`,
  `storageSignDownloadUrl(downloadUrl, expiresSeconds)`, `storageErrorResponse(error)`;
  `type StoredFile`, `type SignedStorageFile`. Ready-made authed routes exist at `/api/storage` (multipart
